...
[intro_bg][logo_i]overlay=(W-w)/2:260:shortest=1,drawtext=fontfile='$fontBoldPath':text='ForamEcoQS':fontcolor=white:fontsize=154:x=(w-text_w)/2:y=1080,drawtext=fontfile='$fontPath':text='GUI tutorial - From abundance matrix to ecological indices':fontcolor=0xD8F5F4:fontsize=66:x=(w-text_w)/2:y=1295,drawtext=fontfile='$fontPath':text='Foraminiferal Ecological Quality Status Assessment':fontcolor=white:fontsize=54:x=(w-text_w)/2:y=1840,fade=t=in:st=0:d=0.6,fade=t=out:st=10.3:d=0.7[intro];
[0:v]trim=start=22:end=196,setpts=PTS-STARTPTS,crop=w='if(between(t,4,16)+between(t,52,64)+between(t,69,82)+between(t,87,115)+between(t,128,150)+between(t,163,173),2048,2560)':h='if(between(t,4,16)+between(t,52,64)+between(t,69,82)+between(t,87,115)+between(t,128,150)+between(t,163,173),1152,1440)':x='if(between(t,25,44),512,if(between(t,52,64)+between(t,69,82)+between(t,87,115)+between(t,163,173),256,0))':y='if(between(t,52,64)+between(t,69,82)+between(t,87,115)+between(t,128,150)+between(t,163,173),144,0)',scale=3840:2160:flags=lanczos,fps=30,subtitles='$assPath',format=yuv420p,fade=t=in:st=0:d=0.35,fade=t=out:st=173.3:d=0.7[workflow];
...
try {
    & $Ffmpeg -y -i $raw -loop 1 -framerate 30 -i $logo -f lavfi -t 11 -i "color=c=0x072B3A:s=3840x2160:r=30" -f lavfi -t 6 -i "color=c=0x072B3A:s=3840x2160:r=30" -filter_complex_script $filter -map "[outv]" -an -c:v libx264 -preset slow -crf 16 -profile:v high -level 5.2 -pix_fmt yuv420p -movflags +faststart $output
