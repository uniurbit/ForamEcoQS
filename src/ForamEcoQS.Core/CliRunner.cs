    ...
    public static class CliRunner
    {
        ...
        public static int Run(string[] args)
        {
            try
            {
                ...
                inputData = ApplyMatrixOrientation(inputData, options.Transpose);
        ...
        private static void ShowHelp()
        {
            ...
            Log("  -transpose            Force transposing the input (use when it has species in columns");
            Log("                        and samples in rows). Omit both flags to auto-detect the layout.");
            Log("  -no-transpose         Never transpose the input, even if it looks the wrong way round.");
        ...
        /// <summary>
        /// Puts the abundance matrix the right way round. Workbooks are just as often written with
        /// species across the header and samples down the first column, which the rest of the CLI
        /// would otherwise read as a single species with a great many samples.
        /// </summary>
        private static DataTable ApplyMatrixOrientation(DataTable inputData, bool? force)
        {
            DataTable oriented = SampleMatrixLayout.EnsureSpeciesInRows(inputData, out var layout, force);

            if (!ReferenceEquals(oriented, inputData))
            {
                Log(force == true
                    ? "Transposing the input as requested (-transpose): species now in rows."
                    : $"The input looks transposed: {layout.Reason} Flipping it so species are in rows " +
                      "(use -no-transpose to keep the file as it is).");
            }
            else if (force != false && layout.Orientation == MatrixOrientation.SpeciesInColumns)
            {
                Log($"Warning: the input may be transposed ({layout.Reason}) but the evidence is weak. " +
                    "Re-run with -transpose if the results look wrong.");
            }
            else if (force != false && layout.Orientation == MatrixOrientation.Unknown && layout.Confidence > 0)
            {
                Log($"Note: {layout.Reason} Assuming species are in rows; use -transpose if that is wrong.");
            }
        ...
        private class CliOptions
        {
            ...
            /// <summary>True: always transpose. False: never. Null: auto-detect the layout.</summary>
            public bool? Transpose { get; set; } = null;
